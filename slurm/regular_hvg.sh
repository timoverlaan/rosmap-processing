...
#SBATCH --output=slurm/out/%j_regular_hvg.out # Set name of output log. %j is the Slurm jobId
#SBATCH --error=slurm/out/%j_regular_hvg.out # Set name of error log. %j is the Slurm jobId
...
for N in 1000 2000 3000; do
    ...
    apptainer exec --writable-tmpfs --pwd /opt/app --containall \
        ...
        ./container_pixi_0-1-3.sif pixi run python -m rosmap_processing pipeline scanpy \
            ...
            data/processed/rosmap_mit_hvg${N}_k30.h5ad \
            --n-genes ${N} \
            --k-neighbors 30

    echo ""
    echo "========================================"
    echo "SeaAD — ${N} HVGs (pre-norm, from ROSMAP_MIT)"
    echo "========================================"
    apptainer exec --writable-tmpfs --pwd /opt/app --containall \
        --bind src/:/opt/app/src/ \
        --bind data/:/opt/app/data/ \
        ...
        ./container_pixi_0-1-3.sif pixi run python -m rosmap_processing pipeline scanpy \
            ...
            data/seaAD/PFC/RNAseq/seaad_hvg${N}_k30.h5ad \
            --layer UMIs \
            --import-genes data/processed/rosmap_mit_hvg${N}_k30.h5ad \
            --k-neighbors 30
done
