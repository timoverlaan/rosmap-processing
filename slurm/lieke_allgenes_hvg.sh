...
#SBATCH --output=slurm/out/%j_lieke_allgenes_hvg.out # Set name of output log. %j is the Slurm jobId
#SBATCH --error=slurm/out/%j_lieke_allgenes_hvg.out # Set name of error log. %j is the Slurm jobId
...
# then select 2000 HVGs on the ROSMAP_MIT training data.
# Step 1: ROSMAP_MIT — intersect with allgenes.txt, then select HVGs within that subset
...
apptainer exec --writable-tmpfs --pwd /opt/app --containall \
	...
	./container_pixi_0-1-3.sif pixi run python -m rosmap_processing pipeline scanpy \
		...
		data/processed/rosmap_mit_lieke_allgenes_hvg2k_k30.h5ad \
		--import-genes data/lieke_allgenes_intersection.txt \
		--hvg-after-import 2000 \
		--k-neighbors 30
...
apptainer exec --writable-tmpfs --pwd /opt/app --containall \
	...
	./container_pixi_0-1-3.sif pixi run python -m rosmap_processing pipeline scanpy \
		...
		data/seaAD/PFC/RNAseq/seaad_lieke_allgenes_hvg2k_k30.h5ad \
		--layer UMIs \
		--import-genes data/processed/rosmap_mit_lieke_allgenes_hvg2k_k30.h5ad \
		--k-neighbors 30
...
extract_genes data/processed/rosmap_mit_lieke_allgenes_hvg2k_k30.h5ad
extract_genes data/seaAD/PFC/RNAseq/seaad_lieke_allgenes_hvg2k_k30.h5ad
